    ...
    create: {
      ...
      parameters: {
        act_as: {
          ...
          required: false,
          format: 'id',
          description: 'The unique ID of the Dev user or the service account to impersonate'
        },
        aud: {
          type: 'array',
          items: 'string',
        ...
        client_id: {
          ...
          required: false,
          format: 'text',
          description: 'An identifier that represents the application which is requesting the token'
        },
        expires_in: {
          type: 'integer',
          required: false,
    ...
    get: {
      ...
      parameters: {
        ...
        include_permissions: {
          type: 'boolean',
          required: false,
          description: 'Whether to include detailed permissions in the response'
        }
      }
    }
  },
    ...
    create: {
      ...
      parameters: {
        ...
        external_refs: {
          ...
          items: 'string',
          required: false,
          description: 'External refs are unique identifiers from your customer system of records'
        },
        owned_by: {
          type: 'array',
          items: 'string',
      ...
      parameters: {
        ...
        stage: {
          ...
          properties: {
            name: {
              type: 'enum',
              allowedValues: ['triage', 'backlog', 'in_progress', 'completed', 'cancelled'],
...
class SpecificationEnhancer {
  ...
  async enhanceAllCollections() {
    console.log('🚀 Enhancing API collections with detailed specifications...\n');
  ...
  async findCollectionFiles(apiName) {
    ...
    for (const dir of dirs) {
      ...
      if ((await fs.stat(dirPath)).isDirectory()) {
        ...
        const collectionFile = dirFiles.find(file => 
  ...
  async enhanceCollectionFile(filePath, operations) {
    ...
    const enhancedPath = filePath.replace('.postman_collection.json', '_Enhanced.postman_collection.json');
  ...
  async enhanceRequest(request, spec) {
    ...
    if (enhanced.url && enhanced.url.query && spec.parameters) {
      enhanced.url.query = this.enhanceQueryParameters(enhanced.url.query, spec.parameters);
    }
  ...
  generateExampleBody(parameters) {
    ...
    for (const [name, param] of Object.entries(parameters)) {
      if (param.type === 'string') {
        ...
        example[name] = param.items === 'string' ? ['example1', 'example2'] : [];
...
if (require.main === module) {
  ...
  enhancer.enhanceAllCollections()
    .then(() => {
